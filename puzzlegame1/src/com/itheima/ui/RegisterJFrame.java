...
public class RegisterJFrame extends JFrame implements MouseListener {
    ...
    public void mouseClicked(MouseEvent e) {
        if (e.getSource()==submit){
            //点击注册
            //用户名,密码不能为空
            if (username.getText()==null||password.getText()==null||rePassword.getText()==null){
                showDialog("用户名或密码不能为空");
                return;
            }
            //判断2次密码是否一致
            if (!password.getText().equals(rePassword.getText())){
                showDialog("输入的两次密码不一致,请重新输入");
                return;
            }
            //判断用户名密码格式是否正确
            if (!username.getText().matches("[a-zA-Z0-9_-]{4,16}")){
                showDialog("用户名格式不正确,应为大写或小写字母数值或下划线,请重新输入");
                return;
            }
            if (!password.getText().matches("\\S*(?=\\S{6,})(?=\\S*\\d)(?=\\S*[a-z])\\S*")){
                showDialog("密码格式错误,至少包含一个数字一个字母,长度至少6位,请重新输入");
                return;
            }
            //判断用户名是否重复
            if (reUsername(username.getText())){
            ...
            //写入文件
            File file=new File("puzzlegame\\userinfo.txt");
